...
export default function CreateCategoryForm() {
  ...
  async function handleCreate(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    if (!name.trim()) {
      toast.error(t('categories.enterName') || 'Please enter the category name', undefined, { anchor });
      return;
    }
    try {
      setLoading(true);
      ...
      toast.error(t('categories.createError') || 'Error creating category', undefined, { anchor });
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="rounded-2xl border border-border bg-card/70 backdrop-blur-md p-4">
      <form className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex-1">
          <label className="mb-1 block text-xs font-medium text-foreground/70">{t('categories.nameLabel') || 'Category name'}</label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder={t('categories.placeholder') || 'e.g.: Technology'}
